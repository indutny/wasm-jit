...
ComboAnalysis.prototype.propagateRanges = function propagateRanges(node,
  ...
  if (/\.const$/.test(node.opcode)) {
    ...
    info.updateRange(new ValueRange(node.literals[0], node.literals[0]));
    return;
  }
  ...
  if (/\.add$/.test(node.opcode)) {
    ...
    info.type = left.type;
    var range = new ValueRange(left.range.from + right.range.from,
  ...
  if (node.opcode === 'ssa:phi') {
    ...
    info.type = left.type;
    var range = new ValueRange(Math.min(left.range.from, right.range.from),
                               Math.max(left.range.to, right.range.to));
    if (info.updateRange(range))
      reducer.change(node);
    return;
  }
