...
InlineParams.prototype.reduceCall = function reduceCall(node, reducer) {
  ...
  // Inputs have fixed register uses, make sure that there is not going to be a
  // conflict
  var replace = new Array(node.inputs.length);
  for (var i = 0; i < node.inputs.length; i++) {
    if (replace[i])
      continue;

    var type = node.literals[i + 1];
    var first = node.inputs[i];
    for (var j = i + 1; j < node.inputs.length; j++) {
      var second = node.inputs[j];
      if (first !== second)
        continue;

      var copy;
      if (type === 'f32' || type === 'f64')
        copy = reducer.graph.create('float.copy');
      else
        copy = reducer.graph.create('int.copy');
      reducer.add(copy);
      copy.addInput(second);
      node.replaceInput(j, copy);
