...
Pipeline.prototype.compile = function compile(masm) {
  ...
  this.reduce(this.cfg);

  var scheduled = Scheduler.create(this.cfg).run();
  return this.compileLow(scheduled, masm);
};

Pipeline.prototype.reduce = function reduce(cfg) {
  var reducer = new Reducer();
  var list = wasm.platform.base.reductions.list;
  list = list.concat(this.platform.reductions.list);
