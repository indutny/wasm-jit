...
builder.opcode('x64:memory.space', function() {
  return { output: this.gp('register') };
}, function(node) {
...
builder.opcode('x64:memory.bounds-check', function() {
  return {
    ...
    inputs: [ this.gp('register'), this.gp('any') ]
  };
}, function(node) {
  var masm = this.masm;
...
builder.opcode('x64:i64.load', function() {
  return {
    output: this.gp('register'),
    inputs: [ this.gp('register'), this.gp('register') ]
  };
}, function(node) {
  var output = this.output(node);
...
builder.opcode('x64:i64.store', function() {
  return {
    inputs: [ this.gp('register'), this.gp('register'), this.gp('register') ]
  };
}, function(node) {
  var space = this.input(node, 0);
  var off = this.input(node, 1);
