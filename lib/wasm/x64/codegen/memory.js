...
builder.opcode('x64:memory.space', function() {
  return { output: this.int('register'), inputs: [ this.int('none') ] };
}, function(node) {
...
// TODO(indutny): eventually sync it up with x64:call
builder.opcode('addr.resize_memory', function() {
  return {
    inputs: [
      // State
      this.int('none'),
  ...
  assert.equal(x64.context, 'rdi');
  assert.equal(size, 'rsi');

  // TODO(indutny): should it be exposed?
  masm.mov('rax', this.table.get('std', 'resize_memory'));
  masm.call('rax');
});
